...
validate_origins <- function(origins, config = read_sampa_config()) {
  ...
  origins$k <- suppressWarnings(as.integer(origins$k))
  ...
  add_error(which(is.na(origins$k) | origins$k < 1 | origins$k > 1000), "k", "range", "k deve estar entre 1 e 1000")
...
#' Geocode one Brazilian CEP using BrasilAPI CEP V2
#'
...
geocode_cep <- function(cep, config = read_sampa_config(), refresh = FALSE) {
  ...
  if (file.exists(cache_path) && !isTRUE(refresh)) return(readRDS(cache_path))
  ...
  coords <- body$location$coordinates
  latitude <- suppressWarnings(as.numeric(coords$latitude %||% NA_real_))
  longitude <- suppressWarnings(as.numeric(coords$longitude %||% NA_real_))
  ...
  result <- data.frame(cep = cep, latitude = latitude, longitude = longitude,
    ...
    geocoder = "BrasilAPI CEP V2 / OpenStreetMap", geocoded_at = utc_now(),
...
resolve_origins <- function(origins, config = read_sampa_config(), refresh_geocodes = FALSE) {
  ...
  for (cep in cep_values) {
    ...
    if (length(cep_values) > 1L) Sys.sleep(delay)
  }
